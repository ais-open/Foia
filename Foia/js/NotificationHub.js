(function () {
    ...
    var _nh = {
        NotificationHub: function (endpoint, sasKey, hubPath) {
            ...
            if (!localSettings.containers.hasKey("Microsot.WindowsAzure.Messaging.WinJSSample")) {
                localSettings.createContainer("Microsot.WindowsAzure.Messaging.WinJSSample", Windows.Storage.ApplicationDataCreateDisposition.always);
            }
            this.container = localSettings.containers.lookup("Microsot.WindowsAzure.Messaging.WinJSSample");
        }
    };

    WinJS.Namespace.define("Microsoft.WindowsAzure.Messaging", _nh);
    ...
    var updateRegistration = function (hub, location, registration) {
        ...
        return new WinJS.xhr({
            ...
            //fix 1.1
            //var location = req.getResponseHeader("Content-Location");
            var location = req.responseText.slice(req.responseText.indexOf('<id>') + 4, req.responseText.indexOf('</id>'));
            //
